...
void init(){
    fstream fin, fin2; 
    fin.open("mixedfullperiods_betas.txt", ios::in); 
...
double evaluate(double t, double x, int i, int index, int k){
    ...
    for (int j = 18; j < N; j += 2) { // epoch*sin and epoch * cos
...
void solve() {
    ...
    for (int i = 0; i < data.size(); ++i){
        for (int j = 0; j < 8; ++j){
            ...
            fout << evaluate(T[index][j-2], data[i][2], i, index, (j-2)*N);
            if (j == 7) fout << '\n';
            else fout << ',';
        }
    }
