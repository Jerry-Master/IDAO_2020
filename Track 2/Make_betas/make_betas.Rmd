...
```{r}
  for(i in 0:599){
    print(i)
     ...
     write(toString(summary(lmX)$coefficients[,1]), file = "betas.txt", append=TRUE)
     write(toString(summary(lmY)$coefficients[,1]), file = "betas.txt", append=TRUE)
     write(toString(summary(lmZ)$coefficients[,1]), file = "betas.txt", append=TRUE)
     write(toString(summary(lmVx)$coefficients[,1]), file = "betas.txt", append=TRUE)
     write(toString(summary(lmVy)$coefficients[,1]), file = "betas.txt", append=TRUE)
     write(toString(summary(lmVz)$coefficients[,1]), file = "betas.txt", append=TRUE)
  }
```
Passar a csv
```{r}
