...
<<<<<<< HEAD
  e <- c()
  for (j in 1:length(ddi$x)) {
    r <- c(ddi$x[j], ddi$y[j], 0)
    v <- c(ddi$Vx[j], ddi$Vy[j], 0)
    t0 <- ddi$epoch[(j-(j%%24))]
    t <- ddi$epoch[j]
    period <- t1
    e <- c(e, eccentricity(r, v))
  ...
  # e <- mean(e)
  # E <- c()
  # for (j in 1:length(testi$x)) {
  #   t0 <- ddi$epoch[1]
  #   t <- testi$epoch[j]
  #   period <- t1
  # 
  #   E <- c(E, eccentric_anomaly(e, mean_anomaly(t, t0, period)))
  # 
  # }
  # testi$E <- E
=======
...
<<<<<<< HEAD
   data.frame(id = testi$id, sat_id=testi$sat_id, epoch=save, x=predict(lmX, testi), y=testi$y_sim, z=predict(lmZ, testi), Vx=predict(lmVx, testi), Vy=predict(lmVy, testi), Vz=predict(lmVz, testi))
=======
   data.frame(id = testi$id, sat_id=testi$sat_id, epoch=testi$epoch, x=predict(lmX, testi), y=predict(lmY, testi), z=predict(lmZ, testi), Vx=predict(lmVx, testi), Vy=predict(lmVy, testi), Vz=predict(lmVz, testi))
>>>>>>> 365b97fe678e0210b1e027879cb364c1cfd412e9
}
